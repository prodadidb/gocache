...
func testPegasusOptions() *store.OptionsPegasus {
	return &store.OptionsPegasus{
...
func TestNewPegasus(t *testing.T) {
	Convey("Pegasus TestNewPegasus should return client and nil error", t, func() {
		...
		p, err := store.NewPegasus(ctx, testPegasusOptions())
		So(err, ShouldBeNil)
		defer func() {
			_ = p.Close()
		}()
...
func Test_validateOptions(t *testing.T) {
	Convey("Pegasus Test validateOptions", t, func() {
		Convey("Test nil validateOptions", func() {
			So(store.ValidatePegasusOptions(&store.OptionsPegasus{}), ShouldNotBeNil)
		})
		Convey("Test correct validateOptions", func() {
			So(store.ValidatePegasusOptions(testPegasusOptions()), ShouldBeNil)
		})
	})
}

func Test_createTable(t *testing.T) {
	Convey("Pegasus Test CreatePegasusTable should return nil", t, func() {
		skipPegasusTest(t)

		ctx := context.Background()

		err := store.CreatePegasusTable(ctx, testPegasusOptions())
		So(err, ShouldBeNil)
	})
}
...
func Test_dropTable(t *testing.T) {
	Convey("Pegasus Test dropTable should return nil", t, func() {
		...
		err := store.DropPegasusTable(ctx, testPegasusOptions())
		So(err, ShouldBeNil)
	})
}
...
func TestPegasusStore_GetWithTTL(t *testing.T) {
	Convey("Pegasus TestGetWithTTL for pegasus store", t, func() {
		...
		Convey("test set ttl that not achieve", func() {
			...
			_ = p.Set(ctx, k, v, store.WithExpiration(retention))
		...
		Convey("test no ttl", func() {
			...
			value, ttl, err := p.GetWithTTL(ctx, k)
			So(cast.ToString(value), ShouldEqual, v)
			So(ttl, should.BeLessThanOrEqualTo, store.PegasusNOTTL)
			So(err, ShouldBeNil)
		})
		Convey("test set ttl that already achieve", func() {
			...
			So(ttl, should.BeLessThanOrEqualTo, store.PegasusNOENTRY)
			So(err, ShouldBeNil)
		})
	})
}
