...
func TestNewRedisCluster(t *testing.T) {
	...
	// When
	s := store.NewRedisCluster(client, store.WithExpiration(6*time.Second))

	// Then
	assert.IsType(t, new(store.RedisClusterStore), s)
	assert.Equal(t, client, s.Clusclient)
...
func TestRedisClusterGet(t *testing.T) {
	...
	s := store.NewRedisCluster(client)
...
func TestRedisClusterGetType(t *testing.T) {
	...
	// When - Then
	assert.Equal(t, store.RedisClusterType, s.GetType())
}
