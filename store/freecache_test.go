...
func TestNewFreecache(t *testing.T) {
	...
	// When
	s := store.NewFreecache(client, store.WithExpiration(6*time.Second))

	// Then
	assert.IsType(t, new(store.FreecacheStore), s)
	assert.Equal(t, client, s.Client)
	assert.Equal(t, &store.Options{
...
func TestFreecacheGetWithTTL(t *testing.T) {
	...
	s := store.NewFreecache(client, store.WithExpiration(3*time.Second))
...
func TestFreecacheGetWithTTLWhenMissingItem(t *testing.T) {
	...
	client := NewMockFreecacheClientInterface(ctrl)
	client.EXPECT().Get([]byte(cacheKey)).Return(nil, store.NotFound{})
...
func TestFreecacheGetWithTTLWhenErrorAtTTL(t *testing.T) {
	...
	client := NewMockFreecacheClientInterface(ctrl)
	client.EXPECT().Get([]byte(cacheKey)).Return(cacheValue, nil)
	client.EXPECT().TTL([]byte(cacheKey)).Return(uint32(0), store.NotFound{})
...
func TestFreecacheSet(t *testing.T) {
	...
	s := store.NewFreecache(client, store.WithExpiration(6*time.Second))
	err := s.Set(ctx, cacheKey, cacheValue, store.WithExpiration(6*time.Second))
	assert.Nil(t, err)
}
...
func TestFreecacheSetWithTags(t *testing.T) {
	...
	s := store.NewFreecache(client, store.WithExpiration(6*time.Second))
	err := s.Set(ctx, cacheKey, cacheValue, store.WithExpiration(6*time.Second), store.WithTags([]string{"tag1"}))
	assert.Nil(t, err)
}
...
func TestFreecacheGetType(t *testing.T) {
	...
	// Then
	assert.Equal(t, store.FreecacheType, ty)
}
